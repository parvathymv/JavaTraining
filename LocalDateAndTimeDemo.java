...
public class LocalDateAndTimeDemo {
	...
	public static void main(String args[]) {
		...
		DateTimeFormatter df = DateTimeFormatter.ofPattern("dd-MMMM-yyyy HH:mm:ss");

		String formatedDateTime = current.format(df);
		System.out.println("in foramatted manner " +formatedDateTime);
		df = DateTimeFormatter.ofPattern("EEEE");
		

		String dayName = current.format(df);
  ...
  System.out.println("current date="+current);
		...
		LocalDate nextTuesday = date.with(TemporalAdjusters.next(DayOfWeek.THURSDAY));
